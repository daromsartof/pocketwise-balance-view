...
const ExpensesSummary: React.FC<ExpensesSummaryProps> = ({
  ...
  const totalExpenses = filteredTransactions
    .filter((t) => t.category.type.toLowerCase() === TransactionType.EXPENSE)
    .reduce((sum, t) => sum + t.amount, 0)

  const totalIncome = filteredTransactions
    .filter((t) => t.category.type.toLowerCase() === TransactionType.INCOME)
    .reduce((sum, t) => sum + t.amount, 0)

  const netAmount = totalIncome - totalExpenses

  // Count transactions by payment method
  const paymentMethodCounts = filteredTransactions.reduce(
    (counts: Record<string, number>, t) => {
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                For period {format(dateRange.startDate, "MMM d, yyyy")} -{" "}
