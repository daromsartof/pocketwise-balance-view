...
const CategoryBreakdown: React.FC<CategoryBreakdownProps> = ({ dateRange }) => {
  ...
  const expenseData = useMemo(() => {
    ...
    return Array.from(categoryMap.entries())
      .map(([categoryId, amount]) => {
        ...
        const percentage = totalExpense > 0 ? (amount as number / totalExpense) * 100 : 0;
  ...
  const incomeData = useMemo(() => {
    ...
    return Array.from(categoryMap.entries())
      .map(([categoryId, amount]) => {
        const category = categories.find(c => c.id === categoryId);
        const percentage = totalIncome > 0 ? (amount as number / totalIncome) * 100 : 0;
...
const CategoryTable: React.FC<CategoryTableProps> = ({ title, data, isExpense }) => {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">{title}</CardTitle>
